...
const CarYearFilter = ({ items }: any) => {
  ...
  const handleChange = (event: any) => {
    if (event.target.checked) {
      addFilters({ key: 'vehicleYear', value: event.target.value });
    } else {
      clearItemFromFilters({ key: 'vehicleYear', value: event.target.value });
    }
  };

  const handleChecked = (name: string) => {
    const params = new URLSearchParams(window.location.search);
    const getYears = params.getAll('vehicleYear');
    return getYears.includes(name);
  };
