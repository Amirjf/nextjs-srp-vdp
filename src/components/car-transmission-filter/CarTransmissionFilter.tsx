...
const CarTransmissionFilter = ({ items }: any) => {
  ...
  const handleChange = (event: any) => {
    if (event.target.checked) {
      addFilters({ key: 'transmission', value: event.target.value });
    } else {
      clearItemFromFilters({ key: 'transmission', value: event.target.value });
    }
  };
  const handleChecked = (name: string) => {
    const params = new URLSearchParams(window.location.search);
    const getTransmissions = params.getAll('transmission');
    return getTransmissions.includes(name);
  };
