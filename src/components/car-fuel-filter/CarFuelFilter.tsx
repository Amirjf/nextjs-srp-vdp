...
const CarFuelFilter = ({ items }: any) => {
  ...
  const handleChange = (event: any) => {
    if (event.target.checked) {
      addFilters({ key: 'fuel_type', value: event.target.value });
    } else {
      clearItemFromFilters({ key: 'fuel_type', value: event.target.value });
    }
  };
  const handleChecked = (name: string) => {
    const params = new URLSearchParams(window.location.search);
    const getFeuls = params.getAll('fuel_type');
    return getFeuls.includes(name);
  };
